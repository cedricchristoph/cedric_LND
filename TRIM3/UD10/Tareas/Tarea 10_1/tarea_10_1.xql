...
<html>
	...
	<table style="border: 1px solid orange;">
		<tr>
			<th bgcolor="orange" style="border: 1px solid orange;">
				CIAL
			</th>
			<th bgcolor="orange" style="border: 1px solid orange;">
				Comentario
		...
		</tr>
		{
			...
			where some $a in $comentario satisfies (contains($comentario, 'No'))
			return
				<tr>
	...
	<ul>
		{
			for $comentario at $count in //comentario
			where contains($comentario, 'CPD')
			return
				<li>Comentario número {string($count)}: {string($comentario/../@cial)} {string($comentario)}</li>
		}
	</ul>
